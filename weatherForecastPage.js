...
bus.on('route:weather:enter', function (city) {
    ...
    if (!myMap) {
        ...
        myMap.events.add('boundschange', debounce(() => {
    ...
    if (city) {
        ...
        fetch('http://ip-api.com/json')
            ...
            .then(function (userinfo) {
                bus.trigger('position:found', [userinfo.lat, userinfo.lon]);
            });
    }

    for (let i = 0; i < favorites.length; i++) {
        favorites[i].point = createPoint(favorites[i].name, favorites[i].lat_lon);
    }
    bus.trigger('favorites:updated');
});
...
function getCityCoordinates(city) {
    return fetch('https://maps.google.com/maps/api/geocode/json?address=' + encodeURIComponent(city) + '&key=AIzaSyDa7DCL2NO9KMPd9DYVk_u3u0wCbm0XXFY')
        .then(function(response) {
            if (response.ok) {
                return response.json();
            }
...
bus.on('weather:update', (lat_lon) => {
    document.getElementById('weather-target').innerHTML = '<img src="./icons/load.gif" width="300"></img>';
    if (document.getElementById('fetch').checked) {
        fetch('https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/b5cc0ae068f1ee7c89f7deff0ae38728/' + lat_lon + '?lang=en&units=si')
        .then(function(response) {
            if (response.ok) {
                return response.json();
            }
        ...
        .catch(function (e) {
            document.getElementById('weather-target').innerHTML = '<img src="./icons/sad.png" width="80"></img><br><p>We are really sorry - the forecast is unavailable right now...</p>';
            console.log('There are some problems with receiving weather! ' + e);
        });
    } else {
        let xhr = new XMLHttpRequest();
        xhr.open('GET', 'https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/b5cc0ae068f1ee7c89f7deff0ae38728/' + lat_lon + '?lang=en&units=si', true);
        xhr.send();
...
function showWheather(weather) {
    ...
    let temp = `<div class ="w"> Temperature: ${w.temperature} <img src="./icons/celsium.png" alt="C" width="15"></img></div>`;
    let sum = `<div class ="w"> Right now: ${w.summary} </div>`;
    let icon = `<img class ="w" src="./icons/${w.icon}.png" alt="${w.icon}" width="50" title="${w.icon}"></img>`;
    ...
    document.getElementById('weather-target').innerHTML = temp + sum + icon + press + hum + wind;
}

bus.on('position:found', (lat_lon) => {
    myMap.panTo(
        lat_lon, { /* Опции перемещения:
